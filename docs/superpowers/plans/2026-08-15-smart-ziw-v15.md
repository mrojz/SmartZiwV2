...
class _FakeOpenAI:
    ...
    def __init__(self, **kwargs):
        self.kwargs = kwargs
        self.chat = MagicMock()
        self.chat.completions.create.return_value = _response('{"ok": true}')
...
def test_lightllm_text_mode_returns_raw_string(monkeypatch):
    monkeypatch.setattr("smart_ziw_llm.OpenAI", _FakeOpenAI)
    _FakeOpenAI.instances[-1].chat.completions.create.return_value = _response("plain answer")
...
def test_lightllm_json_mode_coerces_garbage_to_empty(monkeypatch):
    monkeypatch.setattr("smart_ziw_llm.OpenAI", _FakeOpenAI)
    _FakeOpenAI.instances[-1].chat.completions.create.return_value = _response("no json at all")
    call = getattr(sll, "get_llm_call")({"lightllm_base_url": "http://localhost:8000/v1"})
    assert call("s", "u") == {}
```
